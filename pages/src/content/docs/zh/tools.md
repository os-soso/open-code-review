...
- 每行结果最多保留 **1024 字节**；更长的行在该边界被切断，并标记 `...[truncated]`。读取
  `git grep` 的输出时同时受行数上限与字节上限约束，因此宽泛的搜索不会让工具占用的内存
  随仓库规模增长。
- `search_text` 最多 **1000 个字符**。当 `use_perl_regexp: true` 时，若重复某个分组可能以多种
  方式匹配同一段文本——分组自身含有量词，如 `(a+)+`；或分支不是彼此不同的字面量，如 `(a|aa)+`
  或 `(\w|\d)+`——该模式会被拒绝而不是执行，因为这正是回溯呈指数增长的原因；请改用字符类重写。
  分支为彼此不同字面量的选择（如 `(cat|dog)+`）为线性匹配，会被接受。Perl 正则搜索的超时（5s）
  也比字面量搜索（10s）更短。
- 凭据文件（`.env`、`.ssh/**`、`id_rsa`、`.netrc`、`.npmrc` 以及内置敏感路径黑名单中的其余
  条目）中的匹配不会返回，输出会以
  `Note: Matches in credential files were withheld by the secret-path policy.` 说明这一点。
  被扣留的匹配在读取时即被丢弃，不计入 100 行的上限，因此凭据文件不会占用其后文件的配额。
- 只报告**常规文件**：符号链接及其他非常规路径不会返回，`git grep` 也不会顺着它读取内容。
  工作树中的硬链接在 git 看来就是普通文件，因此它与仓库外的 inode 共享的字节仍可被搜索到——
  审查不可信的 checkout 时，请在隔离的副本中进行。
- 搜索失败时只回报一组固定的消息——例如 `the directory is not a git repository`、
  `git rejected the search pattern`——`git grep` 写出的内容一概不会被复述。它的诊断信息
  会带上 checkout 的绝对路径，并把模型传入的模式原样回显，这些都不会到达模型、会话记录或终端。
  仍然需要把文本带出的两种失败——无法启动 git，或无法读取其输出——会被压平为一行、去掉控制
  字符、截短，并把其中的绝对路径替换为 `<path>`。

...
- 結果の各行は最大 **1024 バイト**までを保持します。それより長い行はその境界で切り詰められ、`...[truncated]` が付きます。
  `git grep` の出力は行数の上限とバイト数の上限の両方の下で読み取られるため、広範な検索でもツールのメモリが
  リポジトリの規模に応じて増えることはありません。
- `search_text` は最大 **1000 文字**です。`use_perl_regexp: true` の場合、グループの繰り返しが同じ文字列を
  複数の方法でマッチしうるパターン——グループ自体が量指定子を含む `(a+)+` のような形、または分岐が互いに異なる
  リテラルではない `(a|aa)+` や `(\w|\d)+` のような形——は実行されずに拒否されます。それがバックトラッキングを
  指数的にする形だからで、文字クラスを使って書き直してください。互いに異なるリテラルの選択（`(cat|dog)+` など）は
  線形時間でマッチするため受け付けられます。Perl 正規表現の検索は、リテラル検索（10s）より
  短いタイムアウト（5s）の下で実行されます。
- 認証情報ファイル（`.env`、`.ssh/**`、`id_rsa`、`.netrc`、`.npmrc`、および組み込みのシークレットパス拒否リストに
  含まれるその他のパス）内のマッチは返されず、出力には
  `Note: Matches in credential files were withheld by the secret-path policy.` と示されます。
  返されないマッチは読み取った時点で破棄され、100 行の上限を消費しないため、認証情報ファイルが
  後続のファイルの取り分を使い切ることはありません。
- 報告されるのは**通常ファイル**のみです。シンボリックリンクなど通常ファイル以外のパスは返されず、`git grep` が
  それをたどって内容を読むこともありません。ワークツリー内のハードリンクは git にとっては普通のファイルなので、
  リポジトリ外の inode と共有しているバイト列は検索可能です——信頼できない checkout は隔離したコピーで
  レビューしてください。
- 検索が失敗した場合は、`the directory is not a git repository` や
  `git rejected the search pattern` といった固定のメッセージ群のいずれかだけが返され、`git grep` が
  書き出した内容がそのまま繰り返されることはありません。git の診断メッセージは checkout の絶対パスを
  含み、モデルが渡したパターンをそのまま echo するため、モデル・セッション記録・ターミナルのいずれにも
  届きません。それでもテキストを持ち出す 2 つの失敗——git を起動できなかった場合と、その出力を読めなかった
  場合——は 1 行に平坦化され、制御文字が除去され、短く切り詰められ、絶対パスは `<path>` に
  置き換えられます。
